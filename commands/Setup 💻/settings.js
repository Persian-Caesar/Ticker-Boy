    ...
    try {
      ...
      interaction.reply({
        ...
        client.on("interactionCreate",async(m)=>{
         try{
          if(!m.user.id === interaction.user.id) return errorMessage(client, m, `This message only for ${interaction.user} and you can't use it.`)
          if(m.isModalSubmit()){
            if(m.customId === "menu_option_modal") {
                const name = interaction.fields.getTextInputValue('name');
                const emoji = interaction.fields.getTextInputValue('emoji');
                if (emoji) await db.push(`guild_${interaction.guild.id}.ticket.menu_option`, {
                  label: name,
                  value: name,
                  emoji: emoji
                })
                else await db.push(`guild_${interaction.guild.id}.ticket.menu_option`, {
                  label: name,
                  value: name
                })
                interaction.update({
                  embeds: [new EmbedBuilder().setTitle(`${client.emotes.system}| Menu Option Setuped`).setColor(client.colors.none).setDescription(`guild **menu option** successfully setuped.\n**Name:** \`${name}\` | **Emoji:** ${emoji ? `${emoji}` : "none"}.`).setFooter({ text: `Setting • Requested By ${interaction.user.tag} `, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) }).setThumbnail(interaction.guild.iconURL({ dynamic: true }))],
