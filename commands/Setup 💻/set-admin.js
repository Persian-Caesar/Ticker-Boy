...
if(message.guild.roles.cache.find(c => c.id === db.fetch(`TicketAdminRole_${message.guild.id}`))) return errorMessage(client, message, `**My Friend, you just have a setup your ticket mod roles befor it to ${message.guild.roles.cache.find(c => c.id === db.fetch(`TicketAdminRole_${message.guild.id}`))}.**`)
