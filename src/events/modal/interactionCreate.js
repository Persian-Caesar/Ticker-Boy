...
module.exports = async (client, interaction) => {
  try {
    ...
    if (interaction.customId === "reporting") {
      ...
      try {
        if (interaction.channel?.permissionsFor(interaction.channel.guild.members.me).has([PermissionsBitField.Flags.CreateInstantInvite])) {
          let i = await interaction.channel.createInvite({ maxAge: 0, maxUses: 5 });
      ...
      webhook.send({
        embeds: [embed],
        components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`report_answer_${interaction.user.id}`).setEmoji(client.emotes.report).setLabel("Answer").setStyle(ButtonStyle.Success).setDisabled(false))],
    ...
    if (interaction.customId === "panel_message_modal") {
      ...
      let embed = new EmbedBuilder()
      let button = new ButtonBuilder().setCustomId('ticket-create').setStyle(ButtonStyle.Success)
